  ...
  // Logs De Consola, Errores y cuentas
  ...
  async function break_captcha(DiscordPage){
    try {
      ...
      write_log(false, "Captcha Encontrado");
      ...
      write_log(true, "Captcha no encontrado");
    };
  }

  async function generate_email(MailPage){
    write_log(true, "Creando Email");
    ...
    try {
      ...
      console.log("Error Encontrado - Generacion De Email");
      return false;
    };
  }

  async function validate_email(MailPage){
    write_log(true, "Verificando Email");
    await MailPage.bringToFront();
  ...
  async function create_accinfos(browser, d) {
    ...
    write_log(true, `Usuario: ${username}`);
    write_log(true, `Contraseña: ${password}`);
    write_log(true, `Email: ${email}`);

    // Crea cuenta | Pasa captcha
    ...
    write_log(true, "Cuenta verificada");

    if(!token){
      write_log(false, "Token no encontrado, Espere un momento")
  ...
  (async () => {
    ...
    try {
      ...
      write_log(true, "Completando la informacion");
      accounts.write(infos + "\n");
    } catch(e) {
      console.log(e);
    } finally {
      write_log(true, "Listo");
      try{
        browser.close();
      } catch(e){};
    }
